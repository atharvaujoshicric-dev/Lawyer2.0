...
window.sendChatMessage=async function(){
  ...
  const payload={firm_id:S.profile.firm_id,sender_id:S.user.id,recipient_id:S.activeChatPeer,body:body||null};
  if(S.pendingChatFile){
    const f=S.pendingChatFile;
    const path=`${S.profile.firm_id}/chat/${Date.now()}_${f.name}`;
...
async function createGroup(){
  ...
  const {data:group,error}=await sb.from('chat_groups')
    .insert({firm_id:S.profile.firm_id,name,created_by:S.user.id}).select().single();
...
async function sendGroupMessage(){
  const groupId=S.activeChatGroup;
  if(!groupId) return;
  const input=document.getElementById('chat-text-input');
  const body=(input.value||'').trim();
  if(!body&&!S.pendingChatFile) return;
  const payload={firm_id:S.profile.firm_id,sender_id:S.user.id,group_id:groupId,recipient_id:null,body:body||null};
  if(S.pendingChatFile){
    const f=S.pendingChatFile;
    const path=`${S.profile.firm_id}/chat/${Date.now()}_${f.name}`;
    const {error:upErr}=await sb.storage.from('lexdesk-files').upload(path,f,{contentType:f.type});
