...
async function addToGroup(){
  ...
  const {error}=await sb.from('chat_group_members').insert({group_id:groupId,user_id:uid,is_admin:false});
  if(error){showToast('Failed: '+error.message,'error');return;}
...
async function removeFromGroup(groupId,uid){
  if(!confirm('Remove this member from the group?')) return;
  await sb.from('chat_group_members').delete().eq('group_id',groupId).eq('user_id',uid);
  await openManageGroupModal();
  showToast('Member removed.','warning');
}
...
async function leaveGroup(){
  ...
  await sb.from('chat_group_members').delete().eq('group_id',groupId).eq('user_id',S.user.id);
