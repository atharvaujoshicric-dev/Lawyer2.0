...
async function generateInvoice(clientId){
  ...
  if(S.invoiceSettings){
    sb.from('invoice_settings').update({next_number:(firm.next_number||1)+1}).eq('firm_id',S.profile.firm_id);
    if(S.invoiceSettings) S.invoiceSettings.next_number = (firm.next_number||1)+1;
  }
}

// ── Invoice settings save (in Settings page) ─────────────────────────────
// invoice_settings has firm_id as its actual primary key (one row per
// firm, created automatically by create_firm_and_admin() at signup), so
// this is always an upsert keyed on firm_id rather than an insert/update
// branch keyed on a separate id column.
async function saveInvoiceSettings(){
  ...
  const {data,error}=await sb.from('invoice_settings').upsert(payload,{onConflict:'firm_id'}).select().single();
