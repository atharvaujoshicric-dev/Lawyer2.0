...
// this only touches <html data-theme> and injects a <style> tag, neither
// of which depends on the partials having loaded yet.
if(typeof initTheme==='function') initTheme();

window.addEventListener('DOMContentLoaded',async ()=>{
  // Wait for assets/js/00-partial-loader.js to finish fetching and
  // injecting every screen/view/modal fragment. Every line below this
  // touches a DOM element (login-screen, app, etc.) that only exists
  // once that promise resolves.
  if(window.__lexdeskPartialsReady){
    try{ await window.__lexdeskPartialsReady; }
    catch(e){ return; } // loader already rendered a fatal-error message
  }
