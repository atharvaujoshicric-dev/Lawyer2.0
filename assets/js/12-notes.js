  ...
  if(S.editingNoteId){
    ...
    if(existing){
      ...
      await sb.from('activity_log').insert({
        firm_id:S.profile.firm_id, actor_id:S.user.id, action:'note_edited',
    ...
    const {data,error}=await sb.from('notes')
      .insert({firm_id:S.profile.firm_id,owner_id:S.user.id,title,content}).select().single();
    if(error){ showToast('Create failed: '+error.message,'error'); return; }
    S.editingNoteId=data.id;
    showToast('Note created.','success');
    await sb.from('activity_log').insert({
      firm_id:S.profile.firm_id, actor_id:S.user.id, action:'note_created',
...
async function addNoteShare(){
  ...
  await sb.from('activity_log').insert({
    firm_id:S.profile.firm_id, actor_id:S.user.id, action:'note_shared',
  ...
  if(sendToChat){
    ...
    await sb.from('messages').insert({
      firm_id:S.profile.firm_id, sender_id:S.user.id, recipient_id:userId, body, note_id:noteId
    });
...
async function updateSharePermission(noteId,userId,perm){
  ...
  await sb.from('activity_log').insert({
    firm_id:S.profile.firm_id, actor_id:S.user.id, action:'note_permission_changed',
    entity_type:'note', entity_id:noteId,
