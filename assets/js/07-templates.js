...
async function saveTemplate(){
  ...
  const payload={firm_id:S.profile.firm_id,name,category:document.getElementById('tpl-category').value||null,content,updated_at:new Date().toISOString()};
...
function loadSettingsUI(){
  ...
  const planRow=document.getElementById('firm-plan-row');
  if(planRow && isAdmin()){
    planRow.style.display='flex';
    const planDesc=document.getElementById('firm-plan-desc');
    if(planDesc && S.firm){
      const planLabel=(S.firm.plan||'trial').charAt(0).toUpperCase()+(S.firm.plan||'trial').slice(1);
      const seatInfo=`${S.users.length}/${S.firm.plan_seats} seats used`;
      const trialInfo=S.firm.plan==='trial' && S.firm.trial_ends_at
        ? ` · Trial ends ${new Date(S.firm.trial_ends_at).toLocaleDateString()}`
        : '';
      planDesc.textContent=`${planLabel} plan · ${seatInfo}${trialInfo}`;
    }
  }
}
...
// NOTE: the old single-tenant build had a checkMigrationStatus() probe here
// to detect whether a customer had manually applied a SQL migration to
// their own Supabase project. In the multi-tenant SaaS model there is
// exactly one shared schema (see database/migrations/) that every firm
// runs against — there's no per-firm migration drift to detect, so this
// function has been removed. Firm-level health (trial expiry, plan
// status, seat limits) is now surfaced via S.firm — see
// src/js/01-supabase-config.js afterAuth().
